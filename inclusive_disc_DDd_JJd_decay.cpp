...
int main(int argc, char** argv) {
        ...
        for (int t = 0; t < head.T;t++) {
            ...
            for (int g1 = 0;g1 < 8;g1++) {
                for (int g2 = 0;g2 < 8;g2++) {
                    int idg1 = 1+g1 + g2 * 8;
                    int id_final = head.ncorr + g1 + g2 * 8;
                    std::complex<double> cp(tmp[0][t][0], tmp[0][t][1]);
                    std::complex<double> cm(tmp[idg1][t][0], tmp[idg1][t][1]);
                    ...
                    std::complex<double>  c4 = sign * cp * cm;
    ...
    for (int g1 = 0;g1 < 8;g1++) {
        for (int g2 = 0;g2 < 8;g2++) {
            int id = head.ncorr+ g1 +g2*8;
            char name[NAMESIZE];
            mysprintf(name,NAMESIZE,"M_%d_%d",g1,g2);
            double *M_i = plateau_correlator_function(
                option, kinematic_2pt, (char*)"P5P5", conf_jack, Njack,
                namefile_plateaux, outfile, id , name, M_eff_T, jack_file);
            free(M_i);
        }
    }
    ...
    for (int mu = 0; mu < 4;mu++) {  // C_munu   mu runs faster // mu is given by the gamma // nu by the insertion
        for (int nu = 0; nu < 4;nu++) {
            ...
            int id_VV =  head.ncorr + (mu + 4) + (nu + 0) * 8;
            int id_AA =  head.ncorr + (mu + 0) + (nu + 4) * 8;
            int id_VA =  head.ncorr + (mu + 4) + (nu + 0) * 8;
            int id_AV =  head.ncorr + (mu + 0) + (nu + 4) * 8;
            ...
            printf("C_%d,%d\n", mu, nu);
            add_correlators(option, ncorr_new, conf_jack, compute_Mmunu_DDd_JJd, fit_info);
            ...
            check_correlatro_counter(67 + (nu + mu * 4) * 3);
            free(Mmunu);
            ...
            check_correlatro_counter(68 + (nu + mu * 4) * 3);
            free(Mmunu_im);

            mysprintf(name, NAMESIZE, "M_M_{%d,%d}", mu, nu);
            double* M_Mmunu = plateau_correlator_function(
                option, kinematic_2pt, (char*)"P5P5", conf_jack, Njack,
                namefile_plateaux, outfile, ncorr_new - 1, name, M_eff_T, jack_file, tmp_info);
            check_correlatro_counter(69 + (nu + mu * 4) * 3);
            free(M_Mmunu);
    ...
    for (int i = 1;i < 6;i++) {
        ...
        check_correlatro_counter(115 + (i - 1) * 3);
        free(Y);
        ...
        check_correlatro_counter(116 + (i - 1) * 3);
        free(Y);

        mysprintf(name, NAMESIZE, "M_Y_{%d}", i);
        double* M_Y = plateau_correlator_function(
            option, kinematic_2pt, (char*)"P5P5", conf_jack, Njack,
            namefile_plateaux, outfile, id_Y[i], name, M_eff_T, jack_file, tmp_info);
        check_correlatro_counter(117 + (i - 1) * 3);
        free(M_Y);
    }
    ...
    for (int i = 0;i < 3;i++) {
        ...
        check_correlatro_counter(130 + i * 3);
        free(Z);
        ...
        check_correlatro_counter(131 + i * 3);
        free(Z);
        mysprintf(name, NAMESIZE, "M_Z_{%d}", i);
        double *M_Z = plateau_correlator_function(
            option, kinematic_2pt, (char*)"P5P5", conf_jack, Njack,
            namefile_plateaux, outfile, id_Z[i], name, M_eff_T, jack_file, tmp_info);
        check_correlatro_counter(132 + i * 3);
        free(M_Z);
    }
