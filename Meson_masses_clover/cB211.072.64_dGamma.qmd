...
fig<- myplotly(gg,"","t", paste0("log Z"), to_print=TRUE )
}
...
mt1<-read_df(file)
all_obs<- get_all_corr(mt1)
# df1<-add_corr_to_df(string =paste0("HLT_",Z,"-sig0.120000-alpha2.00"),
#                     all_obs=all_obs,mt=mt1,df = NULL,
#                    log = FALSE, number = NULL,nudge = 0,print_res = FALSE,logx=2 
#                    )
...
dfA<-get_block(string =paste0("HLT_",Z,"-sig0.120000-alpha2.00"),
                    all_obs=all_obsA,mt=mtA,df = NULL,
                   log = FALSE, number = NULL,nudge = 0,
                    print_res = FALSE,logx=2,ix=2,
                   reshape = FALSE
                   )
...
dfp <- filter(dfA, dfA[,2]>=Amin, dfA[,2]<=Amax )
...
# gg<- gg +scale_x_continuous(breaks = dfA[,1],
#                      labels = dfA[,2])
gg
fig<- myplotly(gg,"","log2(lam)", paste0("rho-th",ths[i]), to_print=TRUE )
```