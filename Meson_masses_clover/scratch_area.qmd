# pessimistic cB211.072.96 $d\Gamma/dq^2$
...
dir<-"/home/garofalo/analysis/heavy_quarks/data_Inc/pessimistic/out/"
ths<- c("1","3","5","7","9")
dG<-rep(0, length(ths))
dG_err<-rep(0, length(ths))
q2<-rep(0, length(ths))
for (i in seq_along(ths) ){
  mt<-read_df(paste0(dir,"cB211.072.96_th",ths[i],"_t56_44.dat_output"))
...
cat("\n## ",Z,"\n\n")
for (i in seq_along(ths) ){
  file<-paste0("/home/garofalo/analysis/heavy_quarks/data_Inc/pessimistic/out/cB211.072.96_th",ths[i],"_t56_44.dat_fit_HLT_",Z,"-sigma")
  ...
  for (s in c(0.120, 0.100, 0.075, 0.050, 0.037, 0.025, 0.017, 0.010, 0.005, 0.0035, 0.002, 0.0005 ) ){
    si<-sprintf("%f",s)
   P<-read_fit_P_file(paste0(dir,"cB211.072.96_th",ths[i],"_t56_44.dat_fit_HLT_",Z,"-sig",si,"-alpha_fit_P.dat"))
   file<-paste0(dir,"cB211.072.96_th",ths[i],"_t56_44.dat_fit_HLT_",Z,"-sig",si,"-alpha_fit_data.txt")
...
df<-NULL
fileA<-paste0("/home/garofalo/analysis/heavy_quarks/data_Inc/pessimistic/out/cB211.072.96_th",ths[i],"_t56_44.dat_HLT_AoverB")
mtA<-read_df(fileA)
...
# file<-paste0("/home/garofalo/analysis/heavy_quarks/data_Inc/out/cB211.072.96_th",ths[i],"_t56_44.dat_output")
# mt1<-read_df(file)
...
df<-NULL
###################### MG
mt1<-read_df("/home/garofalo/analysis/heavy_quarks/data_Inc/pessimistic/out/cB211.072.96_th3_t56_44.dat_HLT_kernel")
all_obs<- get_all_corr(mt1)

df1<-add_corr_to_df(string =paste0("HLT_",Z,"-sig0.000500-alpha-1.99_lam32"),
                    all_obs=all_obs,mt=mt1,df = NULL,
                   log = FALSE, number = NULL,nudge = 0,
                   print_res = FALSE,
                   )
df1<-add_corr_to_df(string =paste0("HLT_",Z,"-sig0.000500-alpha0.00_lam32"),
                    all_obs=all_obs,mt=mt1,df = df1,
                   log = FALSE, number = NULL,nudge = 0,
                   print_res = FALSE)
df1<-add_corr_to_df(string =paste0("HLT_",Z,"-sig0.000500-alpha2.00_lam32"),
                    all_obs=all_obs,mt=mt1,df = df1,
                   log = FALSE, number = NULL,nudge = 0,
                   print_res = FALSE)

df1<-add_corr_to_df(
  string =paste0("HLT_",Z,"-sig0.000500-alpha0.00_lam6.1035156e-05"),
                    all_obs=all_obs,mt=mt1,df = df1,
                   log = FALSE, number = NULL,nudge = 0,
                   print_res = FALSE)
...
df<-NULL
file<-paste0("/home/garofalo/analysis/heavy_quarks/data_Inc/pessimistic/out/cB211.072.96_th",ths[i],"_t56_44.dat_output")
mt1<-read_df(file)
all_obs<- get_all_corr(mt1)
