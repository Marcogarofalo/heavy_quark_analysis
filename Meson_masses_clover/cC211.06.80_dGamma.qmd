...
cat("\n## ",Z,"\n\n")
for (i in seq_along(ths) ){
  file<-paste0("/home/garofalo/analysis/heavy_quarks/data_Inc/out/cC211.06.80_th",ths[i],"_t65_51.dat_fit_HLT_",Z,"-sigma")
...
#gg<- gg + scale_color_manual(values=c("#C04000","#404040","#4863A0"))
# library(scales) # to access break formatting functions
# gg<-gg+scale_x_log10(breaks = trans_breaks("log10", function(x) 10^x),
...
fig<- myplotly(gg,"","log10(A/A0)_ref", paste0("rho-th",ths[i]), to_print=TRUE, 
   ...
   # name<-paste0("C80_",Z,"_th",ths[i],"_","sig",sigmas,".html")
   # namefull<-paste0("./_book/",name)
   # 
   # htmlwidgets::saveWidget(
   #             widget = fig, #the plotly object
   #             file = namefull, #the path & file name
...
df<-NULL
###################### MG
mt1<-read_df("/home/garofalo/analysis/heavy_quarks/data_Inc/out/cC211.06.80_th3_t65_51.dat_HLT_kernel")
all_obs<- get_all_corr(mt1)

df1<-add_corr_to_df(string =paste0("HLT_",Z,"-sig0.005000-alpha-1.99_lam1024"),
                    all_obs=all_obs,mt=mt1,df = NULL,
                   log = FALSE, number = NULL,nudge = 0,
                   print_res = FALSE,
                   )
df1<-add_corr_to_df(string =paste0("HLT_",Z,"-sig0.005000-alpha0.00_lam1024"),
                    all_obs=all_obs,mt=mt1,df = df1,
                   log = FALSE, number = NULL,nudge = 0,
                   print_res = FALSE)
df1<-add_corr_to_df(string =paste0("HLT_",Z,"-sig0.005000-alpha2.00_lam1024"),
                    all_obs=all_obs,mt=mt1,df = df1,
                   log = FALSE, number = NULL,nudge = 0,
                   print_res = FALSE)

df1<-add_corr_to_df(
  string =paste0("HLT_",Z,"-sig0.005000-alpha0.00_lam32"),
                    all_obs=all_obs,mt=mt1,df = df1,
                   log = FALSE, number = NULL,nudge = 0,
                   print_res = FALSE)
